webpackJsonp([7,10,15,16,17,21,27,28],[
/* 0 */
				...
				$(this).hover(function () {
						...
						if (settings.position == "right") {
								...
								if (imageLeft + imageWidth + settings.offset + settings.xzoom > screen.width) {
										...
										var leftpos = imageLeft - settings.offset - settings.xzoom;
								} else {
...
var indexObj = JSON.parse(sessionStorage.getItem('indexObj'));

var packText = '';
var packCount = 0;
// 商品数量加减计算 
$(".goodIntro").on('click', '#add', function () {
    $('#gmNum').val(parseInt($('#gmNum').val()) + 1); //点击加号输入框数值加1
    //显示总金额
});
$(".goodIntro").on('click', '#min', function () {
    $('#gmNum').val(parseInt($('#gmNum').val()) - 1); //点击减号输入框数值减1
    if ($('#gmNum').val() <= 0) {
        $('#gmNum').val(parseInt($('#gmNum').val()) + 1); //最小值为1
    }
});

// 点击切换选项卡
$(".part").on("click", function () {
    $('.part').css({ 'color': '#9c9c9c', 'background': '#f5f5f5' });
    $(this).css({ 'color': 'white', 'background': '#d50000' });
    $(".hideDiv").hide();
    $('.hideDiv').eq($(this).index()).show();
});
// 评价选项卡切换
$('.headList li').on('click', function () {
    $('.headList li a').css({ 'color': '#9c9c9c', 'border-bottom': 'none' });
    $('.headList li a').eq($(this).index()).css({ 'color': '#d50000', 'border-bottom': '2px solid #d50000' });
    $('.contentList li').css("display", "none");
    $('.contentList li').eq($(this).index()).css("display", "block");
});
$(function () {
    //鼠标经过预览图片函数
    $('.uls').on('mousemove', '.preview', function () {
        $(this).parents('.spec-scroll').siblings("#preview").find('img').attr("src", $(this).attr("src"));
        $(this).parents('.spec-scroll').siblings("#preview").find('img').attr("jqimg", $(this).attr("bimg"));
    });
});
$(function () {
...
//销售单位切换颜色
$('.goodIntro').on('click', '.unit span strong', function () {
    $('.unit span strong').css('border', '1px solid #e6e6e6');
...
$('.unit').find('.defaultPack').eq(0).addClass("active");
$('.unit').on('click', '.defaultPack', function () {
    $('.defaultPack').removeClass('active');
...
var url = window.location.href; //获取A链接
var cate = getUrl(url).goods_id;
// 商品详情上半部分
$.ax('/shop-particular-page/' + cate, //（只需要传公共部分后面的路径）
null, //（上送参数 不传为null）
'get', //（方法 传null为post）
'json', //（接受格式 null为json）
function (data) {
    //（成功函数）
    console.log(data);
    console.log(data.result.belong);

    //详情图片插入
    for (var i in data.result.detailed_img) {
        var strImg = '<p><img src="http://shx-admin-api.shenhexin.com/' + data.result.detailed_img[i].img_url + '"><p>';
        $('.Detailpicture').append(strImg);
    }
    //判断来自那个网站
    if (data.result.belong == 'self') {
        if (data.result.master_img != '') {
            var goodPicStr = '<div id="preview" class="spec-preview"> <span class="jqzoom"><img jqimg="http://shx-admin-api.shenhexin.com/' + data.result.master_img[0].img_url + '" src="http://shx-admin-api.shenhexin.com/' + data.result.master_img[0].img_url + '" /></span> </div>' + '<div class="spec-scroll"> <a class="prev">&lt;</a> <a class="next">&gt;</a>' + '<div class="items">' + '<ul class="masterImg">' + '</ul>' + '</div>' + '</div>';
            $('.goodPic').append(goodPicStr);
            for (var _i in data.result.master_img) {
                var master_img = '<li class="uls"><img alt="" bimg="http://shx-admin-api.shenhexin.com/' + data.result.master_img[_i].img_url + '" src="http://shx-admin-api.shenhexin.com/' + data.result.master_img[_i].img_url + '"  class="preview"></li>';
                $('.masterImg').append(master_img);
            }
        } else {
            $('.goodPic').html('<h3>暂无图片</h3>');
        }
        var goodInfor1 = '<div class="goodName" data-goods_id=' + data.result.goods_basic.goods_id + '><h3>' + data.result.goods_basic.goods_name + ' </h3><p> ' + data.result.goods_basic.features + '</p></div>' + '<div class="goodNum">' + '<span>商品编号：</span data-sku=' + data.result.goods_basic.sku + '><b>' + data.result.goods_basic.sku + ' </b>' + '<span>推荐指数：</span><b class="tj"> </b>' + '<span>赠送积分：</span><b>' + data.result.goods_basic.integral_ratio + ' </b>' + '</div>' + '<div class="price">' + '<span>价格：</span><b data-price=' + data.result.goods_basic.shop_price + '>￥ ' + data.result.goods_basic.shop_price + ' </b><span> ' + data.result.goods_basic.unit + ' </span>' + '</div>';
        $('.goodIntro').append(goodInfor1);
        for (var num = 0; num <= data.result.goods_basic.reco_degree; num++) {
            $('.tj').append('⭐');
        }
        $('.price').after('<div class="unit"><span>销售单位：</span></div>');
        $('.unit').after('<div class="spec"><span>商品规格：</span></div>');
        for (var a in data.result.pack) {
            var unitMeg = '<strong class="unitStrong" data-count=' + data.result.pack[a].count + '> ' + data.result.pack[a].name + '</strong>';
            $('.unit span').append(unitMeg);
        }
        //判断是否有规格
        if (data.result.goods_spec != '') {
            for (var _a in data.result.goods_spec) {
                var _unitMeg = '<a href="' + indexObj.details + '?goods_id=' + data.result.goods_spec[_a].goods_id + '"><strong class="defaultPack" data-goods_id=' + data.result.goods_spec[_a].goods_id + '> ' + data.result.goods_spec[_a].spec + '</strong></a>';
                $('.spec span').append(_unitMeg);
            }
        }
        var cart = '<div class="num">' + '<span>购买数量：</span>' + '<button id="min">-</button>' + '<input type="text" value="1" id="gmNum" >' + '<button id="add">+</button>' + '<span style="margin-left:20px">已销售：</span><b>' + data.result.goods_basic.sales_num + '</b>' + '</div>' + '<div class="buts">' + '<button class="carts">加入购物车</button>' + '<button class="collection">收藏商品</button>' + '</div>';
        $('.spec').after(cart);
        ...
        $(function () {
            if (localStorage.getItem('token') != null) {
                //获取单位
                packText = $('.unitStrong').eq(0).text();
                packCount = $('.unitStrong').eq(0).data('count');
                console.log(packText);
                $('.goodIntro').on('click', '.unit span strong', function () {
                    packText = $(this).text();
                    packCount = $(this).data('count');
                });
                $('.goodIntro').on('click', '.carts', function () {
                    var num = $('#gmNum').val();
                    var price = $('.price b').data('price');
                    var amount = num * price;
                    console.log(packText);
                    $.ax("/shopping-cart-insert", //（只需要传公共部分后面的路径）
                    {
                        num: $('#gmNum').val(),
                        price: $('.price b').data('price') * packCount,
                        amount: amount,
                        goods_id: $('.goodName').data('goods_id'),
                        name: $('.goodName h3').text() + '/' + packText
                    }, //（上送参数 不传为null）
                    'POST', //（方法 传null为post）
                    null, //（接受格式 null为json）
                    function (data) {
                ...
                $('.goodIntro').on('click', '.carts', function () {
                    ...
                    var url = $('.jqzoom img').attr("src");
                    var sum = $('.price b').text().split('￥')[1] * $('#gmNum').val() * packCount;
                    var price = $('.price b').text().split(' ');
                    var keyName = "cart" + $('.goodName').data('goods_id');
                    var goods = { goods_id: $('.goodName').data('goods_id'), goodname: $('.goodName').text() + '/' + packText, price: price[1] * packCount, num: $('#gmNum').val(), img: url, sum: sum };
                    var data = JSON.stringify(goods); //转字符串
    ...
    //网易严选添加商品详情
            ...
            if (data.result.master_img != '') {
                var _goodPicStr = '<div id="preview" class="spec-preview"> <span class="jqzoom"><img jqimg="' + data.result.master_img[0].img_url + '" src="' + data.result.master_img[0].img_url + '" /></span> </div>' + '<div class="spec-scroll"> <a class="prev">&lt;</a> <a class="next">&gt;</a>' + '<div class="items">' + '<ul class="masterImg">' + '</ul>' + '</div>' + '</div>';

                $('.goodPic').append(_goodPicStr);
                for (var _i2 in data.result.master_img) {
                    var _master_img = '<li class="uls"><img  src="' + data.result.master_img[_i2].img_url + '"  class="preview"></li>';
                    $('.masterImg').append(_master_img);
                }
            }
            var goodInfor1 = '<div class="goodName" data-goods_id=' + data.result.goods_basic.goods_id + '><h3>' + data.result.goods_basic.goods_name + ' </h3><p> ' + data.result.goods_basic.features + '</p></div>' + '<div class="goodNum">' + '<span>商品编号：</span data-sku=' + data.result.goods_basic.sku + '><b>' + data.result.goods_basic.sku + ' </b>' + '<span>推荐指数：</span><b class="tj"> </b>' + '<span>赠送积分：</span><b>' + data.result.goods_basic.integral_ratio + ' </b>' + '</div>' + '<div class="price">' + '<span>价格：</span><b data-price=' + data.result.goods_basic.shop_price + '>￥ ' + data.result.goods_basic.shop_price + ' </b><span> ' + data.result.goods_basic.unit + ' </span>' + '</div>';
            $('.goodIntro').append(goodInfor1);
            for (var _num = 0; _num <= data.result.goods_basic.reco_degree; _num++) {
                $('.tj').append('⭐');
            }
            $('.price').after('<div class="unit"><span> 销售单位：</span></div>');
            $('.unit').after('<div class="spec"><span>商品规格：</span></div>');
            var _unitMeg2 = '<strong class="unitStrong" > ' + data.result.goods_basic.unit + '</strong>';
            $('.unit span').after(_unitMeg2);
            if (data.result.goods_spec != '') {
                for (var _a2 in data.result.goods_spec) {
                    var _unitMeg3 = '<a href="' + indexObj.details + '?goods_id=' + data.result.goods_spec[_a2].goods_id + '"><strong class="defaultPack" data-goods_id=' + data.result.goods_spec[_a2].goods_id + '> ' + data.result.goods_spec[_a2].spec + '</strong></a>';
                    $('.spec span').append(_unitMeg3);
                }
            }
            var _cart = '<div class="num">' + '<span>购买数量：</span>' + '<button id="min">-</button>' + '<input type="text" value="1" id="gmNum" >' + '<button id="add">+</button>' + '<span style="margin-left:20px">已销售：</span><b>' + data.result.goods_basic.sales_num + '</b>' + '</div>' + '<div class="buts">' + '<button class="carts">加入购物车</button>' + '<button class="collection">收藏商品</button>' + '</div>';
            $('.spec').after(_cart);
            //购物车添加商品 登录时候添加购物车
            ...
            $(function () {
                if (localStorage.getItem('token') != null) {
                    $('.goodIntro').on('click', '.carts', function () {
                        ...
                        var price = $('.price b').data('price');
                        var amount = num * price;
                        var packClick = $.ax("/shopping-cart-insert", //（只需要传公共部分后面的路径）
                        {
                            num: $('#gmNum').val(),
                            price: $('.price b').data('price'),
                            amount: amount,
                            goods_id: $('.goodName').data('goods_id'),
                            name: $('.goodName h3').text()
                        }, //（上送参数 不传为null）
                        'POST', //（方法 传null为post）
                        null, //（接受格式 null为json）
                        function (data) {
                    ...
                    $('.goodIntro').on('click', '.carts', function () {
                        var storage = window.sessionStorage;
                        var url = $('.jqzoom img').attr("src");
                        var sum = $('.price b').text().split('￥')[1] * $('#gmNum').val();
                        console.log(sum);
                        // var keyName = "cart" + $('.goodName').data('goods_id');
                        var goods = { goods_id: $('.goodName').data('goods_id'), sku: $('.goodName span').data('sku'), goodname: $('.goodName').text(), price: $('.price b').text(), num: $('#gmNum').val(), img: url, sum: sum };
                        var data = JSON.stringify(goods); //转字符串
                        storage.setItem(keyName, data);
                        keyName++;
                        alert('添加购物车成功！！');
    ...
    //判断规格使边框变颜色
    for (var _i3 = 0; _i3 <= $('.spec span a').children('.defaultPack').length; _i3++) {
        console.log($('.spec span').children('.defaultPack').eq(_i3).data('goods_id'));
        if (data.result.goods_basic.goods_id == $('.spec span a').children('.defaultPack').eq(_i3).data('goods_id')) {
            $('.spec span a').children('.defaultPack').eq(_i3).css('border', '2px solid red');
        }
    }
}, function (e) {});
