...
;(function ($, window, document, undefined) {
			...
			$.DialogByZ = {
						...
						//自动显示弹框
						Autofade: function Autofade(params) {
...
$('.enter').on("click", function () {
    ...
    var textHidd = $('.hiddOne'); //用户名
    var passHidd = $('.hiddTwo'); //密码提示框
    if (text.val() == '' && pass.val() == '') {
        textHidd.html('用户名不能为空');
        passHidd.html('密码不能为空');
        text.addClass("color");
        pass.addClass("color");
        return false;
    } else if (text.val() == '') {
        ...
        passHidd.html('');
        $('.enter').html('登陆中……');
        $.ax("/shop-login", //（只需要传公共部分后面的路径）
        { 'employee_num': text.val(),
            'password': pass.val() }, //（上送参数 不传为null）
        'post', //（方法 传null为post）
        'json', //（接受格式 null为json）
        function (data) {
            //（成功函数）
