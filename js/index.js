webpackJsonp([4,10,13,15,16,17,27,28],[
/* 0 */
...
//获取首页分类
var svnArry = ['#icon-bangongyongpin', '#icon-bangong', '#icon-diannao', '#icon-icon--', '#icon-weibiaoti526', '#icon-iconset0119', '#icon-icon-test', '#icon-unie637'];
$.ax("/shop-cate-list", //（只需要传公共部分后面的路径）
null, //（上送参数 不传为null）
'get', //（方法 传null为post）
null, //（接受格式 null为json）
function (data) {
    ...
    for (var i in data.result) {
        cates += "<li>" + "<a href='" + indexObj.classlist + "?cate_id=" + data.result[i].cate_id + "&level=" + data.result[i].level + "'>" + "<svg class='icon' aria-hidden='true'>" + "<use xlink:href='" + svnArry[i] + "'></use>" + "</svg>&nbsp" + data.result[i].cate_name + "</a>" + "<div class='hideDiv'>";

        for (var q in data.result[i].children) {
            cates += "<ul>" + "<li class='twoLi'>";
            cates += "<a href='" + indexObj.classlist + "?cate_id=" + data.result[i].children[q].cate_id + "&level=" + data.result[i].children[q].level + "'><b>" + data.result[i].children[q].cate_name + "</b></a>" + "</li>";
            cates += "<li class='thrLi'>";
            for (var t in data.result[i].children[q].children) {
                cates += "<a href='" + indexObj.classlist + "?cate_id=" + data.result[i].children[q].children[t].cate_id + "&level=" + data.result[i].children[q].children[t].level + "'>" + data.result[i].children[q].children[t].cate_name + "</a>";
            }
            cates += "</li>" + "</ul>";
        }
        "</div>" + "</li>";
    }
    $('.left').find('ul').append(cates);
}, function (data) {//（失败函数）
...
// 商城首页楼层请求
$.ax("/shop-index-floor", //（只需要传公共部分后面的路径）
null, //（上送参数 不传为null）
'get', //（方法 传null为post）
'json', //（接受格式 null为json）
function (data) {
    //（成功函数）
    console.log(data);
    if (data.message == '无商品信息') {
        $('.rightMeg').html(data.message);
    } else {

        for (var i in data.result) {
            var rightBox = 'rightBox' + i;
            var mabox = '<div class="computer" id="mouse' + i + '">' + '<h2>' + data.result[i].sort_num + 'F<span>' + data.result[i].name + '</span></h2>' + '<ul class="computerList ">' + '</ul>' + '<div class="clear"></div>' + '<div class="main">' + '<div class="leftMain">' + '<a href="' + data.result[i].pic_url_1 + '"><img src="http://shx-admin-api.shenhexin.com/' + data.result[i].layout_pic_1 + '"></a>' + '</div><div class="rightMain rightBox' + i + '"></div></div>' + '</div>';
            $('.lcbox').append(mabox);
            for (var a in data.result[i].class.names) {
                var titleName = '<li data-cate_id=' + data.result[i].class.names[a].cate_id + '>' + data.result[i].class.names[a].cate_name + '</li>';
                $('#mouse' + i + ' .computerList').append(titleName);
            }
            for (var b in data.result[i].class.goods_info) {
                var rightMeg = '<div class="good margin10" data-goods_id=' + data.result[i].class.goods_info[b].goods_id + '>' + '<a href="' + indexObj.details + '?goods_id=' + data.result[i].class.goods_info[b].goods_id + '" class="img"><img src="' + data.result[i].class.goods_info[b].small_pic + '" class="imgScale"></a>' + '<p class="goodsName"><a href="' + indexObj.details + '?goods_id=' + data.result[i].class.goods_info[b].goods_id + '">' + data.result[i].class.goods_info[b].goods_name + '</a></p>' + '<p>￥' + data.result[i].class.goods_info[b].shop_price + '</p>' + '</div>';

                $('.' + rightBox).append(rightMeg);
            }
        }
    }
}, function (data) {
...
$('.lcbox').on('click', '.computerList li', function () {
    $(this).parent().parent().index();
    console.log($(this).parent().parent().index());
    console.log('aaaaa');
    var str = 'mouse' + $(this).parent().parent().index() + ' .computerList li';
    var str1 = 'rightBox' + $(this).parent().parent().index();
    // console.log(str);
    var rightMain = 'rightBox' + $(this).parent().parent().index();
    $('#' + str).css({ 'color': 'black', 'border-bottom': '2px solid #f7f7f7' });
    $(this).css({ 'color': '#d50000', 'border-bottom': '2px solid #d50000' });
    //楼层导航切换ajax 请求
    var cate_id = $(this).data('cate_id');
    $('.' + str1).children('div').remove();
    $.ax("/shop-index-floor-info/" + cate_id, //（只需要传公共部分后面的路径）
    null, //（上送参数 不传为null）
    'get', //（方法 传null为post）
    'json', //（接受格式 null为json）
    function (data) {
        ...
        for (var i in data.result) {
            var rightMeg = '<div class="good margin10" data-goods_id=' + data.result[i].goods_id + '>' + '<a href="' + indexObj.details + '?goods_id=' + data.result[i].goods_id + '" class="img"><img src="' + data.result[i].small_pic + '" class="imgScale"></a>' + '<p class="goodsName"><a href="javascript:void(0)">' + data.result[i].goods_name + '</a></p>' + '<p>￥' + data.result[i].shop_price + '</p>' + '</div>';
            $('.' + rightMain).append(rightMeg);
        }
    }, function (data) {
        //（失败函数）
        console.log(data);
...
if (localStorage.getItem('token') != null) {
    ...
    for (var _i in stoArry) {
        $.ax('/shopping-cart-insert', {
            goods_id: stoArry[_i].goods_id,
            name: stoArry[_i].goodname,
            amount: stoArry[_i].sum,
            price: stoArry[_i].price,
            num: stoArry[_i].num
