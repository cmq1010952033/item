webpackJsonp([2,10,11,15,16,17,22,24,27,28],[
/* 0 */
...
var cate = getUrl(url).cate_id;
var level = getUrl(url).level;
var company_id = getUrl(url).company_id;
$.ax('/shop-class-page/' + cate + '/' + level, //（只需要传公共部分后面的路径）
null, //（上送参数 不传为null）
'get', //（方法 传null为post）
null, //（接受格式 null为json）
function (data) {
    ...
    if (data.success != 'false') {
        ...
        for (var i = 0, len = dataObj.length; i < len; i += 12) {
            DataArry.push(dataObj.slice(i, i + 12));
        }
        ...
        $('#box').paging({
            ...
            callback: function callback(page) {
                // 回调函数
                $('.listClass ul').children('li').remove();
                for (var _i2 in DataArry[page - 1]) {
                    var goodsInfo = '<li>' + '<div class="listImg"><a href="' + indexObj.details + '?goods_id=' + DataArry[page - 1][_i2].goods_id + '"><img src="http://shx-admin-api.shenhexin.com/"' + DataArry[page - 1][_i2].img_url + '></a></div>' + '<p class="title_list"><a href="' + indexObj.details + '?goods_id=' + DataArry[page - 1][_i2].goods_id + '">' + DataArry[page - 1][_i2].goods_name + '</a></p>' + '<p class="price_list"><span class="classPrice">￥' + parseFloat(DataArry[page - 1][_i2].shop_price).toFixed(2) + '</span>/' + DataArry[page - 1][_i2].unit + '</p>' + '<div class="bot_list"><div class="collection">☆收藏</div><div class="collection">□ 对比</div><div class="goodsCart" data-goods_id=' + DataArry[page - 1][_i2].goods_id + '><img src="../../img/go.png">加入购物车</div></div>' + '</li>';
                    $(".listClass ul").append(goodsInfo);
                }
            }
        });
    } else {
        $('.listClass ul').html('该分类下无商品信息');
    }
}, function (data) {
    //（失败函数）
    console.log(data);
...
$(function () {
    if (localStorage.getItem('token') != null) {
        $('.listClass').on('click', '.goodsCart', function () {

            var price = $(this).parent().prev().children('.classPrice').text().split('￥');
            var goods_id = $(this).data('goods_id');
            var name = $(this).parent().prevAll('.title_list').text();
            console.log(name);
        ...
        $('.listClass').on('click', '.goodsCart', function () {
            ...
            var goods_id = $(this).data('goods_id');
            var name = $(this).parent().prevAll('.title_list').text();
            var url = $(this).parent().prevAll('.listImg').children('img').attr('src');
            var keyName = "cart" + goods_id;
            var goods = { goods_id: goods_id, goodname: name, price: price[1], num: 1, img: url, sum: price[1] };
