webpackJsonp([5,10,13,15,16,22,27,28],[
/* 0 */
...
$(function () {
    ...
    console.log(base64Obj);
    // 单选默认第一选中
    $('.pay').eq(0).addClass('active');
    $('.isOpen').eq(0).addClass('active');
    $('.type').eq(0).addClass('active');

    $.ax('/shop-admin-address-list/' + base64Obj.employee_id, //（只需要传公共部分后面的路径）
    null, 'get', //（方法 传null为post）
    null, //（接受格式 null为json）
    function (data) {
        //（成功函数）
        var res = data.result,
        ...
        for (var i = 0; i < res.length; i++) {
            html += '<p class="per"><span class="perName">' + res[i].user_name + '</span> ' + ' <span class="perSxq">' + res[i].area_name[0] + res[i].area_name[1] + res[i].area_name[2] + '</span> ' + ' <span class="perStreet">' + res[i].street + '</span>' + ' <span class="perTel">' + res[i].phone + '</span>' + '</p>';
        }
        $('.receiver').html(html);
        $('.per').eq(0).addClass('active');
        $('.name').html($('.per').eq(0).find('.perName').html());
        $('.address').html($('.per').eq(0).find('.perSxq').html() + $('.per').eq(0).find('.perStreet').html());
        $('.tel').html($('.per').eq(0).find('.perTel').html());
        $('.per').click(function () {
            $('.per').attr('class', 'per');
            $(this).addClass('active');
            $('.name').html($(this).eq(0).find('.perName').html());
            $('.address').html($(this).eq(0).find('.perSxq').html() + $(this).eq(0).find('.perStreet').html());
            $('.tel').html($(this).eq(0).find('.perTel').html());
        });
    }, function (data) {
        //（失败函数）
        console.log(data);
    ...
    $('.submit').click(function () {
        ...
        $.ax('/shopping-cart-order-insert', //（只需要传公共部分后面的路径）
        {
            company_id: base64Obj.company_id,
            shopping_cart_id: window.localStorage.getItem("shopid"),
            remark: $('.remark').val(),
            name: $('.name').html(),
            address: $('.address').html(),
            pay_method: $('.payType').find('.active').html()
        }, 'post', //（方法 传null为post）
        null, //（接受格式 null为json）
        function (data) {
            //（成功函数）
