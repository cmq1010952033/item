webpackJsonp([1,10,11,13,15,16,22,27,28],[
/* 0 */
...
$('.nameListUl').on('click', '.twoLlist', function () {

	//获取二级分类得商品
	$.ax('/yanxuan-goods-list-query/' + $(this).data('cate_id'), //（只需要传公共部分后面的路径）
	null, //（上送参数 不传为null）
	'get', //（方法 传null为post）
	null, //（接受格式 null为json）
	function (data) {
		...
		var DataArry = [];
		var dataObj = [];
		for (var _i in data.result) {
			dataObj[_i] = data.result[_i];
		}
		for (var i = 0, len = dataObj.length; i < len; i += 8) {
			DataArry.push(dataObj.slice(i, i + 8));
		}
		//分页初始化
		$('#box').paging({
			initPageNo: 1, // 初始页码
			totalPages: DataArry.length, //总页数
			slideSpeed: 600, // 缓动速度。单位毫秒
			jump: true, //是否支持跳转
			callback: function callback(page) {
				// 回调函数
				$('.spList ul').children('li').remove();
				for (var _i2 in DataArry[page - 1]) {
					var mrShop = '<li>' + '<div class="listImg">' + '<a href="' + indexObj.details + '?goods_id=' + DataArry[page - 1][_i2].goods_id + '"><img src="' + DataArry[page - 1][_i2].url_img + '" alt=""></a>' + '</div>' + '<div class="lsitWord">' + '<div class="listTop">' + '<div class="topLeft"><a href="' + indexObj.details + '?goods_id=' + DataArry[page - 1][_i2].goods_id + '">' + DataArry[page - 1][_i2].goods_name + '</a></div>' + '<div class="topRight"><a href="' + indexObj.details + '?goods_id=' + DataArry[page - 1][_i2].goods_id + '">￥' + DataArry[page - 1][_i2].shop_price + '</a></div>' + '</div>' + '<div class="listButton">' + DataArry[page - 1][_i2].features + '' + '</div>' + '</div>' + '</li>';
					$('.spList ul').append(mrShop);
				}
			}
		});
	});
...
$(function () {
	//获取分类
	$.ax("/yanxuan-category-list-query/0", //（只需要传公共部分后面的路径）
	null, //（上送参数 不传为null）
	'get', //（方法 传null为post）
	null, //（接受格式 null为json）
	function (data) {
		...
		for (var i in data.result) {
			if (i < 6) {
				...
				var twoLlist = '<li class="twoLlist"  data-cate_id="' + data.result[i].cate_id + '"data-level="' + data.result[i].level + '"data-pid="' + data.result[i].pid + '">' + data.result[i].cate_name + '' + '</li>';
				$('.nameListUl').append(twoLlist);
			}
		}
		$('.moerBox').click(function () {
			for (var _i3 in data.result) {
				if (_i3 > 5) {

					var _twoLlist = '<li class="twoLlist"  data-cate_id="' + data.result[_i3].cate_id + '"data-level="' + data.result[_i3].level + '"data-pid="' + data.result[_i3].pid + '">' + data.result[_i3].cate_name + '' + '</li>';
					$('.nameListUl').append(_twoLlist);
					$('.moer').hide();
				}
			}
		});
		//获取默认商品
		var cate_id = $('.twoLlist').eq(0).data('cate_id');
		$.ax('/yanxuan-goods-list-query/' + cate_id, //（只需要传公共部分后面的路径）
		null, //（上送参数 不传为null）
		'get', //（方法 传null为post）
		null, //（接受格式 null为json）
		function (data) {
			...
			var DataArry = [];
			var dataObj = [];
			for (var _i4 in data.result) {
				dataObj[_i4] = data.result[_i4];
			}
			for (var i = 0, len = dataObj.length; i < len; i += 8) {
				DataArry.push(dataObj.slice(i, i + 8));
			}
			$('#box').paging({
				...
				callback: function callback(page) {
					// 回调函数
					$('.spList ul').children('li').remove();
					for (var _i5 in DataArry[page - 1]) {
						var mrShop = '<li>' + '<div class="listImg">' + '<a href="' + indexObj.details + '?goods_id=' + DataArry[page - 1][_i5].goods_id + '"><img src="' + DataArry[page - 1][_i5].url_img + '" alt=""></a>' + '</div>' + '<div class="lsitWord">' + '<div class="listTop">' + '<div class="topLeft"><a href="' + indexObj.details + '?goods_id=' + DataArry[page - 1][_i5].goods_id + '">' + DataArry[page - 1][_i5].goods_name + '</a></div>' + '<div class="topRight"><a href="' + indexObj.details + '?goods_id=' + DataArry[page - 1][_i5].goods_id + '">￥' + DataArry[page - 1][_i5].shop_price + '</a></div>' + '</div>' + '<div class="listButton">' + DataArry[page - 1][_i5].features + '' + '</div>' + '</div>' + '</li>';
						$('.spList ul').append(mrShop);
					}
				}
			});
...
/***************************************************点击二级分类获取三级分类*********************************************************************************************************/
//更多得三级分类
$('.moerList').on('click', '.MtwoList', function () {
	$('.threeLlistbox').show();
});
$('.hide').click(function () {
	$('.threeLlistbox').hide();
...
$('.nameListUl').on('click', '.twoLlist', function () {
	$('.threeLlistbox').show();
	$('.twoLlist').css('color', '#000');
	$(this).css('color', '#b0882f');
	$.ax('/yanxuan-category-list-query/' + $(this).data('cate_id'), //（只需要传公共部分后面的路径）
	null, //（上送参数 不传为null）
	'get', //（方法 传null为post）
	null, //（接受格式 null为json）
	function (data) {
		//（成功函数）
		$('.threeLlistboxUl').children('li').remove();
		for (var i in data.result) {
			var threeLlist = '<li class="threeLlist"  data-cate_id="' + data.result[i].cate_id + '"data-level="' + data.result[i].level + '"data-pid="' + data.result[i].pid + '">' + data.result[i].cate_name + '' + '</li>';
			$('.threeLlistboxUl').append(threeLlist);
		}
	});
});

/*********************************************************点击三级分类获取商品列表*********************************************************************************************************/
$('.threeLlistboxUl').on('click', '.threeLlist', function () {
	$('.threeLlist').css('color', '#000');
	$(this).css('color', '#b0882f');
	//获取二级分类得商品
	$.ax('/yanxuan-goods-list-query/' + $(this).data('cate_id'), //（只需要传公共部分后面的路径）
	null, //（上送参数 不传为null）
	...
	function (data) {
		...
		$('#box').paging({
			...
			callback: function callback(page) {
				// 回调函数
				$('.spList ul').children('li').remove();
				for (var _i7 in DataArry[page - 1]) {
					var mrShop = '<li>' + '<div class="listImg">' + '<a href="' + indexObj.details + '?goods_id=' + DataArry[page - 1][_i7].goods_id + '"><img src="' + DataArry[page - 1][_i7].url_img + '" alt=""></a>' + '</div>' + '<div class="lsitWord">' + '<div class="listTop">' + '<div class="topLeft"><a href="' + indexObj.details + '?goods_id=' + DataArry[page - 1][_i7].goods_id + '">' + DataArry[page - 1][_i7].goods_name + '</a></div>' + '<div class="topRight"><a href="' + indexObj.details + '?goods_id=' + DataArry[page - 1][_i7].goods_id + '">￥' + DataArry[page - 1][_i7].shop_price + '</a></div>' + '</div>' + '<div class="listButton">' + DataArry[page - 1][_i7].features + '' + '</div>' + '</div>' + '</li>';
					$('.spList ul').append(mrShop);
				}
			}
		});
