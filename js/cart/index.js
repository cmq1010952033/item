webpackJsonp([6,10,13,15,16,22,27,28],[
/* 0 */
...
$(".recommendList").find("li").on("click", function () {
    $(".recommendList").find("li").removeClass("active");
    $(this).addClass("active");
    $(".recommendList").find(".hideDiv").hide();
    $(this).find(".hideDiv").show();
});
//没有登陆得时候添加购物车商品
function shoeCate() {
    var storage = window.sessionStorage; //获取本地存储
    delete storage.headerObj;
    delete storage.indexObj;
    delete storage.navObj;
    var stoArry = [];
    for (var i = 0; i < storage.length; i++) {
        stoArry.push(JSON.parse(storage.getItem(storage.key(i))));
    }
    if (storage.length > 0) {
        for (var _i in stoArry) {
            console.log(stoArry[_i].goods_id);
            var cartLlist = '<tr class="goodInfor" data-goods_id=' + stoArry[_i].goods_id + '>' + '<td class="Check"><input type="checkbox" name="boxCheck" class="boxCheck"></td>' + '<td class="infor">' + '<img src="http://shx-admin-api.shenhexin.com/' + stoArry[_i].img_url + '">' + '<span>' + stoArry[_i].goodname + '</span>' + '</td>' + '<td class="price">￥' + stoArry[_i].price * 1 + '</td>' + '<td class="num">' + '<input type="button" value="-" class="min"><input type="number"id="numberInput" value="' + stoArry[_i].num + '"><input type="button" value="+" class="add">' + '</td>' + '<td class="totalPrice">￥' + stoArry[_i].sum + '</td>' + '<td class="operation"><span>已收藏</span> | <label class="cartDelete">移除</label></td>' + '</tr>';
            $('.cartNullBox').remove();
            $('.cartTable').append(cartLlist);
        }
    } else {
        var cartStr = '<div class="cartNullBox">' + '<div class="cartSvg">' + '<svg class="icon" aria-hidden="true">' + '<use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-gouwuche"></use>' + '</svg>' + '</div>' + '<div class="cartNull">' + '<p>您的购物车空空如也~~赶快去购物吧！！</p>' + '</div>' + '</div>';
        $('.goodList').html(cartStr);
    }
}
//判断是否登陆
$(function () {
    if (localStorage.getItem('token') != null) {
        //购物车列表数据请求
        $.ax("/shopping-cart-query", //（只需要传公共部分后面的路径）
        null, //（上送参数 不传为null）
        'get', //（方法 传null为post）
        'json', //（接受格式 null为json）
        function (data) {
            ...
            if (data.success == 'true') {
                for (var i in data.result) {
                    var cartLlist = '<tr class="goodInfor" data-cart_id=' + data.result[i].shopping_cart_id + '>' + '<td class="Check"><input type="checkbox" name="boxCheck" class="boxCheck"></td>' + '<td class="infor">' + '<img src="' + data.result[i].img_url + '">' + '<span>' + data.result[i].name + '</span>' + '</td>' + '<td class="price">￥' + data.result[i].price + '</td>' + '<td class="num">' + '<input type="button" value="-" class="min"><input type="number"id="numberInput" value="' + data.result[i].num + '"><input type="button" value="+" class="add">' + '</td>' + '<td class="totalPrice">￥' + data.result[i].amount + '</td>' + '<td class="operation"><span>已收藏</span> | <label class="cartDelete">移除</label></td>' + '</tr>';
                    $('.cartNullBox').remove();
                    $('.cartTable').append(cartLlist);
                }
            }
            $('.shopAll').html(data.result.length);
        ...
        //购物车删除数据
        $('.goodList').on('click', '.cartDelete', function () {
            var _this = this;

            var id = $(this).parent().parent().data('cart_id');
            $.ax('/shopping-cart-delete/' + id, //（只需要传公共部分后面的路径）
            null, //（上送参数 不传为null）
            'DELETE', //（方法 传null为post）
            'json', //（接受格式 null为json）
            function (data) {
                //（成功函数）
                console.log(data);
            ...
            var pre = $(this).parent().prev().text().split('￥');
            cartSum = cartSum - pre[1] * 1;
            $('.sum').html(cartSum.toFixed(2));

            // 判断静态下清空购物车
            if ($('.goodInfor').length == 1) {
                var cartStr = '<div class="cartNullBox">' + '<div class="cartSvg">' + '<svg class="icon" aria-hidden="true">' + '<use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-gouwuche"></use>' + '</svg>' + '</div>' + '<div class="cartNull">' + '<p>您的购物车空空如也~~赶快去购物吧！！</p>' + '</div>' + '</div>';
                $('.goodList').html(cartStr);
            }
        });
        ...
        $(".goodList").on('click', '.min', function () {
            ...
            $(this).next().val(parseFloat($(this).next().val()) - 1); //点击减号输入框数值减1
            if ($(this).next().val() <= 0) {
                $(this).next().val(parseFloat($(this).next().val()) + 1); //最小值为1
            }
            ...
            var id = $(this).parent().parent().data('cart_id');
            //购物车减数据请求
            $.ax("/shopping-cart-update", //（只需要传公共部分后面的路径）
            {
                num: num,
                shopping_cart_id: id
            }, //（上送参数 不传为null）
            ...
            function (data) {
                //（成功函数）
                var sum = num * pricetwo[1];
                var sumMain = 0;
                $(_this2).parent().next().html('￥' + sum.toFixed(2));
                // if (num >=1){
                //     if ($(this).parent().siblings('.Check').children('.boxCheck').is(":checked") == true) {
                //         cartSum = cartSum - pricetwo[1] * 1;
                //         console.log(cartSum)
                //         $('.sum').html(cartSum.toFixed(2))
                //     } 
                // }
                $('input[name="boxCheck"]:checked').each(function () {
                    if ($('input[name="boxCheck"]:checked').is(":checked") == true) {
                        var a = $(this).parent().nextAll('.totalPrice').text().split('￥');
                        sumMain = sumMain + a[1] * 1;
                        cartSum = sumMain;
                        $('.sum').html(cartSum.toFixed(2));
                    }
                });
        ...
        $(".goodList").on('click', '.add', function () {
            ...
            $(this).prev($('#numberInput')).val(parseFloat($(this).prev($('#numberInput')).val()) + 1);
            var price = $(this).parent().prev().text();
            var pricetwo = price.split("￥");
            var num = $(this).prev($('#numberInput')).val();
            var id = $(this).parent().parent().data('cart_id');
            $.ax("/shopping-cart-update", //（只需要传公共部分后面的路径）
            {
                num: num,
            ...
            function (data) {
                ...
                console.log(data);
                var sum = num * pricetwo[1];
                $(_this3).parent().next().html('￥' + sum.toFixed(2));
                if ($(_this3).parent().siblings('.Check').children('.boxCheck').is(":checked") == true) {
                    cartSum = cartSum + pricetwo[1] * 1;
                    console.log(cartSum);
                    $('.sum').html(cartSum.toFixed(2));
                } else {}
        ...
        // 购物车数量
        $(".goodList").on('click', '.min', function () {
            var num = $(this).next().val(parseFloat($(this).next().val()) - 1); //点击减号输入框数值减1
            if ($(this).next().val() <= 0) {
                $(this).next().val(parseFloat($(this).next().val()) + 1); //最小值为1
            }
            var price = $(this).parent().prev().text();
            var pricetwo = price.split("￥");
            var num = $(this).next().val();
            var goodname = $(this).parent().prevAll('.infor').children('span').text();
            var ulr = $(this).parent().prevAll('.infor').children('img').attr('src');
            var goodsId = $(this).parent().parent().data('goods_id');
            console.log(goodsId);
            var goods = { goods_id: goodsId, goodname: goodname, price: pricetwo[1], num: num, img: ulr, sum: parseFloat(num * pricetwo[1]).toFixed(2) };
            var data = JSON.stringify(goods); //转字符串
            sessionStorage.setItem('cart' + goodsId, data);

            $(this).parent().next().html('￥' + parseFloat(num * pricetwo[1]).toFixed(2));
        });
        $(".goodList").on('click', '.add', function () {
            $(this).prev($('#numberInput')).val(parseFloat($(this).prev($('#numberInput')).val()) + 1);
            var price = $(this).parent().prev().text();
        ...
        $('.goodList').on('click', '.cartDelete', function () {
            ...
            sessionStorage.removeItem('cart' + $(this).parent().parent().data('goods_id'));
        });
        //未登录的选中商品删除
        $('.allDelete').click(function () {

            $('input[name="boxCheck"]:checked').each(function () {
                var cart_id = $(this).parent().parent().data('goods_id');
                console.log(cart_id);
                sessionStorage.removeItem('cart' + cart_id);
                $(this).parent().parent().remove();
            });
            console.log($('.goodList table tbody').children('.goodInfor').length);
            cartNum = 0;
            cartSum = 0;
            $('.shopChoice').text('0');
            $('.sum').html('￥' + 0);
            if ($('.goodList table tbody').children('.goodInfor').length <= 0) {
                $('.goodList table tbody').children('tr').remove();
                $(this).parent().parent().remove();
                var cartStr = '<div class="cartNullBox">' + '<div class="cartSvg">' + '<svg class="icon" aria-hidden="true">' + '<use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-gouwuche"></use>' + '</svg>' + '</div>' + '<div class="cartNull">' + '<p>您的购物车空空如也~~赶快去购物吧！！</p>' + '</div>' + '</div>';
                $('.goodList').html(cartStr);
                $('.shopAll').html('0');
...
$('.settlement').on('click', function () {
    ...
    if (tokenStr != null) {
        ...
        $('input[name="boxCheck"]:checked').each(function () {
            ...
            var cart_id = $(this).parent().parent().data('cart_id');
            console.log(cart_id);
            shopId += '' + cart_id + ',';
        });
        //获取默认地址

        var shopIdStr = shopId.slice(0, shopId.length - 1); //去掉后面得，号
        console.log('购物车ID    ' + shopIdStr);
        if (shopId != '') {
            localStorage.setItem('shopid', shopIdStr);
            window.location.href = '/view/form/index.html';
        } else {
            alert('你还没有添加商品');
        }
    } else {
        alert('还没有登陆点击确定跳转登陆界面');
        $(window).attr('location', '../../view/login/index.html');
    }
});
...
$('input[name="allcheck"]').click(function () {
    ...
    if ($(this).is(':checked') == true) {
        ...
        console.log('aaaaa');
        cartSum = 0;
        $('input[name="boxCheck"]').each(function (index) {
            //此处如果用attr，会出现第三次失效的情况  
            $(this).prop("checked", true);

            var sum = $('.totalPrice').eq(index).text().split('￥');

            cartSum = cartSum + sum[1] * 1;

            $('.sum').html(cartSum.toFixed(2));
            cartNum = index + 1;
            $('.shopChoice').html(cartNum);
        });
    } else {
...
var babm = localStorage.getItem('token');

// 购物车购买商品总件数和总价格计算

$('.cartTable').on('click', '.boxCheck', function () {
    if ($(this).is(":checked") == true) {
        cartNum++;
        var xj = $(this).parent().siblings('.totalPrice').text().split('￥');
        cartSum = xj[1] * 1 + cartSum;
        // console.log('总数： '+ cartSum.toFixed(2))
    } else {
        cartNum--;
        var xj = $(this).parent().siblings('.totalPrice').text().split('￥');
        cartSum = cartSum - xj[1] * 1;
        console.log(cartSum.toFixed(2));
    }

    $('.shopChoice').text(cartNum);
    $('.sum').html('￥' + cartSum.toFixed(2));
});
//删除选中的商品
$('.allDelete').click(function () {
    ...
    console.log($('.goodList table tbody').children('.goodInfor').length);
    $.ax('/shopping-cart-delete/' + id.slice(0, id.length - 1), //（只需要传公共部分后面的路径）
    null, //（上送参数 不传为null）
    'DELETE', //（方法 传null为post）
    null, //（接受格式 null为json）
    function (data) {
        ...
        if (data.success == 'true') {
            ...
            cartSum = 0;
            $('input[name="boxCheck"]:checked').each(function () {
                if ($('input[name="boxCheck"]').is(':checked')) {
                    $(this).parent().parent().remove();
                    $('.shopAll').html($('.goodList table tbody').children('.goodInfor').length);
                    $('.shopChoice').text('0');
                    $('.sum').html('￥' + 0);
                }
            });
            ...
            if ($('.goodList table tbody').children('.goodInfor').length <= 0) {
                ...
                console.log($('input[name="boxCheck"]').length);
                alert(data.message);
                var cartStr = '<div class="cartNullBox">' + '<div class="cartSvg">' + '<svg class="icon" aria-hidden="true">' + '<use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-gouwuche"></use>' + '</svg>' + '</div>' + '<div class="cartNull">' + '<p>您的购物车空空如也~~赶快去购物吧！！</p>' + '</div>' + '</div>';
                $('.goodList').html(cartStr);
                $('.shopAll').html('0');
